<script lang="ts">
	import { elevators, systemStatus, connectionStatus, elevatorUtilization, pendingRequests, addNotification } from '../../stores/elevators';
...
<div class="h-full flex flex-col">
	...
	<div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
		...
		<button
			...
			class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm font-medium transition-colors flex items-center gap-2"
		>
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
			...
			<div class="flex items-center space-x-3">
				<div class="flex items-center">
					<div class="h-3 w-3 rounded-full {$connectionStatus.connected ? 'bg-green-500' : 'bg-red-500'} mr-2"></div>
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
			...
			<div class="space-y-3">
				<div class="flex items-center justify-between">
					...
					<span class="text-sm font-medium {$systemStatus.healthy ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}">
						{$systemStatus.healthy ? 'Healthy' : 'Issues Detected'}
					</span>
				</div>
				<div class="flex items-center justify-between">
					<span class="text-sm text-gray-600 dark:text-gray-400">Total Elevators</span>
					<span class="text-sm font-medium text-gray-900 dark:text-white">{$systemStatus.elevatorCount}</span>
				</div>
				<div class="flex items-center justify-between">
					<span class="text-sm text-gray-600 dark:text-gray-400">Operational</span>
					<span class="text-sm font-medium text-green-600 dark:text-green-400">{healthyElevators}</span>
				</div>
				<div class="flex items-center justify-between">
			...
			<div class="space-y-3">
				<div>
					<div class="flex justify-between text-sm mb-1">
						...
						<span class="text-gray-900 dark:text-white">{$elevatorUtilization.idle.toFixed(1)}%</span>
					</div>
					<div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
						<div class="bg-green-500 h-2 rounded-full" style="width: {$elevatorUtilization.idle}%"></div>
					</div>
				</div>
				<div>
					<div class="flex justify-between text-sm mb-1">
						<span class="text-gray-600 dark:text-gray-400">Active</span>
						<span class="text-gray-900 dark:text-white">{$elevatorUtilization.moving.toFixed(1)}%</span>
					</div>
					<div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
						<div class="bg-blue-500 h-2 rounded-full" style="width: {$elevatorUtilization.moving}%"></div>
					</div>
				</div>
				<div>
					<div class="flex justify-between text-sm mb-1">
						<span class="text-gray-600 dark:text-gray-400">Error</span>
						<span class="text-gray-900 dark:text-white">{$elevatorUtilization.error.toFixed(1)}%</span>
					</div>
					<div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
						<div class="bg-red-500 h-2 rounded-full" style="width: {$elevatorUtilization.error}%"></div>
					</div>
				</div>
			</div>
		</div>
					...
					{#each $elevators as elevator}
						<div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
							<div class="flex items-center justify-between mb-2">
								<span class="text-sm font-medium text-gray-900 dark:text-white">{elevator.name}</span>
								<div class="flex items-center space-x-2">
									<div class="h-2 w-2 rounded-full {elevator.status === 'idle' ? 'bg-green-500' : elevator.status === 'moving' ? 'bg-blue-500' : 'bg-red-500'}"></div>
									<span class="text-xs text-gray-600 dark:text-gray-400 capitalize">{elevator.status}</span>
								</div>
							</div>
